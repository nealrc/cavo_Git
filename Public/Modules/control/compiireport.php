...
$d = $_GET['id'];
$query = "SELECT `name` from `base_school` WHERE `id`=".$d;
...
$query = "SELECT `Userid`, `Firstname`, `Lastname`, `EnrollmentYear` from `user` WHERE `Membership`=4 AND `University`=".$d;
$result = mysql_query($query, $cavoconnection) or die(mysql_error());
$rows = mysql_fetch_assoc($result);
$total_nums = mysql_num_rows($result);	
if($total_nums >0){
	do{
		$nn = ucfirst(strtolower($rows['Firstname'])).' '.ucfirst(strtolower($rows['Lastname']));
		$users[$rows['EnrollmentYear']][$nn] = $rows['Userid'];
	}while($rows = mysql_fetch_assoc($result));
	krsort($users);
}
mysql_free_result($result);
...
<title>CAVO Comprehensive Report for Institution</title>
<!-- InstanceEndEditable -->
...
<div id="container">
    <div id='content-wrapper'>
        ...
        <h1>CAVO Comprehensive Report for <span class='thighlight'><?php echo $school; ?></span></h1>
        ...
        <div class='chartwrapper' id='s_<?php echo $d;?>'></div>
        
        <h2>Select students</h2>
        
        <div id='yrlist'>
			<?php if(count($users)>0){foreach($users as $year => $arr){?>
            
            <h3><a href='#'>Enrollemnt year <?php echo $year;?></a></h3>            
            <ul>
            <?php $tarr=$arr;ksort($tarr);foreach($tarr as $name=>$id){?>
            <li><a href='compreport.php?id=<?php echo $id;?>'><?php echo $name;?></a></li>
            <?php } ?>
            </ul>
