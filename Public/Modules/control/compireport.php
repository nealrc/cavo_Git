...
if($_SESSION['MM_UserGroup']==$role['admin']){
	$query = "SELECT a.`University` AS id, b.`name` AS school from `user` AS a 
			LEFT JOIN `base_school` AS b ON (a.`University`=b.`id`)";		
}elseif($_SESSION['MM_UserGroup']==$role['instructor']){
	$query = "SELECT a.`University` AS id, b.`name` AS school from `user` AS a 
			LEFT JOIN `base_school` AS b ON (a.`University`=b.`id`) 
			WHERE a.`Userid`=".$_SESSION['MM_Userid'];
}
...
if($total_nums >0){
	do{
		if(!empty($rows['school']) && $rows['school'] !=NULL){
...
<div id="container">
    <div id='content-wrapper'>
        ...
        <h1>CAVO Comprehensive Report for Institute</h1>
        
        <h2>Select institute</h2>
        <ul>        
        <?php if(count($school)>0){foreach($school as $id => $name){?>
        <li><a href='compiireport.php?id=<?php echo $id;?>'><?php echo $name;?></a></li>
        <?php }}else{?>
        <li>No institute found!</li> 
        <?php } ?>
        </ul>      
