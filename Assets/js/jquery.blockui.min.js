...
if(b.browser.msie||a.forceIframe)l.css("opacity",0);g=[l,h,f];var y=e?b("body"):b(c);b.each(g,function(){this.appendTo(y)});a.theme&&a.draggable&&b.fn.draggable&&f.draggable({handle:".ui-dialog-titlebar",cancel:"li"});g=z&&(!b.boxModel||b("object,embed",e?null:c).length>0);if(r||g){e&&a.allowBodyStretch&&b.boxModel&&b("html,body").css("height","100%");if((r||!b.boxModel)&&!e){g=parseInt(b.css(c,"borderTopWidth"))||0;i=parseInt(b.css(c,"borderLeftWidth"))||0;var s=g?"(0 - "+g+")":0,t=i?"(0 - "+i+")":
0}b.each([l,h,f],function(A,B){var j=B[0].style;j.position="absolute";if(A<2){e?j.setExpression("height","Math.max(document.body.scrollHeight, document.body.offsetHeight) - (jQuery.boxModel?0:"+a.quirksmodeOffsetHack+') + "px"'):j.setExpression("height",'this.parentNode.offsetHeight + "px"');e?j.setExpression("width",'jQuery.boxModel && document.documentElement.clientWidth || document.body.clientWidth + "px"'):j.setExpression("width",'this.parentNode.offsetWidth + "px"');t&&j.setExpression("left",
t);s&&j.setExpression("top",s)}else if(a.centerY){e&&j.setExpression("top",'(document.documentElement.clientHeight || document.body.clientHeight) / 2 - (this.offsetHeight / 2) + (blah = document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop) + "px"');j.marginTop=0}else if(!a.centerY&&e){var C="((document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop) + "+(a.css&&a.css.top?parseInt(a.css.top):0)+') + "px"';j.setExpression("top",
C)}})}if(d){a.theme?f.find(".ui-widget-content").append(d):f.append(d);if(d.jquery||d.nodeType)b(d).show()}if((b.browser.msie||a.forceIframe)&&a.showOverlay)l.show();if(a.fadeIn){g=a.onBlock?a.onBlock:o;l=a.showOverlay&&!d?g:o;g=d?g:o;a.showOverlay&&h._fadeIn(a.fadeIn,l);d&&f._fadeIn(a.fadeIn,g)}else{a.showOverlay&&h.show();d&&f.show();a.onBlock&&a.onBlock()}u(1,c,a);if(e){k=f[0];m=b(":input:enabled:visible",k);a.focusInput&&setTimeout(v,20)}else D(f[0],a.centerX,a.centerY);if(a.timeout){d=setTimeout(function(){e?
